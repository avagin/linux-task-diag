...
/**
 * seq_put_hex_ll - print a number in hexadecimal notation
 * @m: seq_file identifying the buffer to which data should be written
 * @delimiter: a string which is printed before the number
 * @v: the printed number
 ...
 * In usual cases, it will be better to use seq_printf(). It's easier to read.
 */
void seq_put_hex_ll(struct seq_file *m, const char *delimiter, unsigned long long v, int width)
{
	int i, len;

	seq_put_delimeter(m, delimiter);

        len = __builtin_clzll(v);
        len = (64 - len + 3) / 4;
	...
	for (i = 0; i < len; i++) {
		m->buf[m->count + len - 1 - i] = hex_asc[0xf & v];
		v = v >> 4;
	}
	m->count += len;
	return;
