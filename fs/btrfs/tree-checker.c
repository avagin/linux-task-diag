				 ...
				 int slot)
{
	...
	if (unlikely(!IS_ALIGNED(key->objectid, blocksize))) {
		generic_err(leaf, slot,
		"free space info key objectid is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
	if (unlikely(!IS_ALIGNED(key->offset, blocksize))) {
		generic_err(leaf, slot,
		"free space info key offset is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
				   ...
				   int slot)
{
	...
	if (unlikely(!IS_ALIGNED(key->objectid, blocksize))) {
		generic_err(leaf, slot,
		"free space extent key objectid is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
	if (unlikely(!IS_ALIGNED(key->offset, blocksize))) {
		generic_err(leaf, slot,
		"free space extent key offset is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
				   ...
				   struct btrfs_key *key, int slot)
{
	...
	if (unlikely(!IS_ALIGNED(key->objectid, blocksize))) {
		generic_err(leaf, slot,
		"free space bitmap key objectid is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
	if (unlikely(!IS_ALIGNED(key->offset, blocksize))) {
		generic_err(leaf, slot,
		"free space bitmap key offset is not aligned to %u, has (%llu %u %llu)",
			    blocksize, key->objectid, key->type, key->offset);
		return -EUCLEAN;
	}
	if (unlikely(key->offset == 0)) {
