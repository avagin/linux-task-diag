...
int main(int argc, char *argv[])
{
	...
	if (argc > 1 && strcmp(argv[1], "verify") == 0) {
		unsigned long hwcap_type = strtoul(argv[2], NULL, 16);
		unsigned long expected = strtoul(argv[3], NULL, 16);
		unsigned long hwcap = getauxval(hwcap_type);

		if (hwcap != expected) {
			ksft_print_msg("HWCAP mismatch: expected %s, got %lx\n", argv[2], hwcap);
			return 1;
		}
	...
	if (size == -1)
		ksft_exit_fail_perror("PR_GET_AUXV");

	count = size / sizeof(unsigned long);

	/* Find the "latest" feature and try to maks it out. */
